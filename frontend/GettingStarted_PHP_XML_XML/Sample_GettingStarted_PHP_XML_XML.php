...
$query = 'kühlschrank';  // Supply your own query keywords as needed
...
if ($resp->ack == "Success") {
  ...
  foreach($resp->searchResult->item as $item) {
    ...
    $itemId = $item->itemId;
    $energyEfficientClass = $item->eekStatus;
    $pic   = $item->galleryURL;
    $link  = $item->viewItemURL;
    $title = $item->title;

    if($item->primaryCategory->categoryId == "71262" && $item->condition->conditionId == "1000") {
      $items[] = $item;
    }  
...
function constructPostCallAndGetResponse($endpoint, $query, $xmlfilter) {
  ...
  $xmlrequest .= "<paginationInput>\n <entriesPerPage>300</entriesPerPage>\n</paginationInput>\n";
